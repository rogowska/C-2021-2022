...
int generate_next_state(int borning_rule, int surviving_rule, int neighbours, int row_index, int column_index, int **matrix)
...
deciding the state of a cell based on the number of its neighbours and the previous state*/
{
    ...
    {
        if (logic_rules(neighbours,borning_rule))
        {
            return 1;
        }
        else
    ...
    {
        if (logic_rules(neighbours,surviving_rule))
        {
            return 1;
        }
        else
